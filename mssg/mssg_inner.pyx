...
from libcpp.algorithm cimport copy, sort
...
REAL = np.float32
DEF RANDOM_INT_MAX_VALUE = 4_294_967_295
...
cdef unsigned long long w2v_fast_sentence_sgcon_neg(
    const int negative, np.uint32_t *cum_table, unsigned long long cum_table_len,
    REAL_t *sensevectors, REAL_t *syn0, const int size, const np.uint32_t word_index,
    const np.uint32_t word2_index, const REAL_t alpha, REAL_t *work, REAL_t *neu1, REAL_t *neu2, 
    ...
    """
    cdef long long a
    cdef long long row1 = word_index * size, row2, rowt
    cdef unsigned long long modulo = 281474976710655ULL
    cdef REAL_t f, fc, g, mg, gc, label, f_dot, fc_dot, log_e_f_dot, back_ti, sum_dti
    cdef REAL_t x_l2, y_l2, xg, yg, sim, exp_sim, blog_sim, bias_sum, bias_sumc
    cdef REAL_t alpha_f, beta_f, gamma_f, sim_l2, front, back_a, back_b
    cdef np.uint32_t target_index
    cdef int d, i, j
...
#     memset(neu1, 0, size * cython.sizeof(REAL_t)) 
    ...
    for d in range(negative+1):
        if d == 0:
            target_index = word2_index
            label = ONEF
        else:
            target_index = bisect_left(cum_table, (next_random >> 16) % cum_table[cum_table_len-1], 0, cum_table_len)
            next_random = (next_random * <unsigned long long>25214903917ULL + 11) & modulo
            if target_index == word2_index:
                continue
            label = <REAL_t>0.0

        # cos_sim negative label = -1
        row2 = target_index * size 
        f_dot = our_dot(&size, &sensevectors[row1], &ONE, &syn0[row2], &ONE) #+ bias0[word2_index] + bias1neg[target_index]
        if f_dot <= -MAX_EXP or f_dot >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f_dot + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha# * 5.0
        our_saxpy(&size, &g, &syn0[row2], &ONE, work, &ONE)
        our_saxpy(&size, &g, &sensevectors[row1], &ONE, &syn0[row2], &ONE)
            
    # normal dot
    our_saxpy(&size, &words_lockf[word2_index % lockf_len], work, &ONE, &sensevectors[row1], &ONE)
...
cdef init_w2s_config(Word2VecConfig *c, model, alpha, compute_loss, _work, _syn1neg, _neu1, _neu2, _ave_con):
    ...
    c[0].syn0 = <REAL_t *>(np.PyArray_DATA(model.wv.vectors)) # global (negative)
...
#         c[0].syn1neg = <REAL_t *>(np.PyArray_DATA(model.syn1neg))
    ...
    c[0].ave_con = <REAL_t *>np.PyArray_DATA(_ave_con)

    
    c[0].np_value = model.np_value
    c[0].sensevectors = <REAL_t *>(np.PyArray_DATA(model.wv.sensevectors))
    c[0].clustervectors = <REAL_t *>(np.PyArray_DATA(model.wv.clustervectors))
    c[0].skip_oov = model.skip_oov    
    c[0].index_to_sensenum = <np.int8_t *>(np.PyArray_DATA(model.wv.index_to_sensenum))
    c[0].index_to_maxsensenum = <np.int8_t *>(np.PyArray_DATA(model.wv.index_to_maxsensenum))
    c[0].sensecounts = <np.uint32_t *>(np.PyArray_DATA(model.wv.sensecounts)) 
...
def train_batch_sg(model, sentences, alpha, _work, _syn1neg, _neu1, _neu2, _ave_con, compute_loss):
    ...
    cdef int i, j, k, l, m, n, cluster_index, context_size, csr_check, include_check, edge_index
    cdef np.uint32_t sense_index, target_sense_index = 0
    cdef int effective_words = 0, effective_sentences = 0
    cdef int sent_idx, idx_start, idx_end, new_sensenum
    cdef REAL_t window_size = 0, sum_topm = 0
    cdef REAL_t sim = 0, max_sim = 0, min_sim = 0
    cdef REAL_t f_dot, f, g
    cdef np.uint32_t *vocab_sample_ints
    cdef np.uint32_t padding_idx = len(model.wv.key_to_index) + 1 # zero padding 用の index
    cdef np.uint32_t padding_senseidx = len(model.wv.key_to_senseindex) + 1 # zero padding 用の index
#     init_w2s_config(&c, model, alpha, compute_loss, _work)
    init_w2s_config(&c, model, alpha, compute_loss, _work, _syn1neg, _neu1, _neu2, _ave_con)
    if c.sample:
    ...
    for sent in sentences:
        ...
        for token in sent:
            if token not in model.wv.key_to_index:
                if not model.skip_oov:
                    word_index = padding_idx # unsigned int だと負の数がないので， # leaving `effective_words` unchanged = shortening the sentence = expanding the window
                    sense_index = padding_senseidx
                    sense_num = 0
                else:
                    continue
            else:
                word_index = model.wv.key_to_index[token]
                sense_index = model.wv.key_to_senseindex[token]
                if c.hs:
            ...
            c.senseindexes[effective_words] = sense_index
            effective_words += 1
    ...
    for i in range(effective_words):
        if c.sample and vocab_sample_ints[c.indexes[i]] < random_int32(&c.next_random):
            c.indexes[i] = padding_idx
#             c.index_samples[i] = vocab_sample_ints[c.indexes[i]] / RANDOM_INT_MAX_VALUE
    ...
    with nogil:
        for sent_idx in range(effective_sentences):
            ...
            for i in range(idx_start, idx_end):
                if c.indexes[i] == padding_idx: continue
                ...
                memset(c.ave_con, 0, c.size * cython.sizeof(REAL_t))
                memset(c.neu1, 0, c.size * cython.sizeof(REAL_t))
# #                 memset(c.dcluster_vectors, 0, c.cluster_num * c.size * cython.sizeof(REAL_t))

                window_size = 0.0
                # simple average
                for l in range(j, k):
                    if l == i: continue
                    if c.indexes[l] == padding_idx: continue
                    window_size += ONEF
                    our_saxpy(&c.size, &ONEF, &c.syn0[c.indexes[l] * c.size], &ONE, c.ave_con, &ONE)
                window_size = ONEF / window_size
            
                max_sim = -1000
                sense_index = 0
                for l in range(c.index_to_sensenum[c.indexes[i]]):
                    sim = our_dot(&c.size, &c.clustervectors[(c.senseindexes[i]+l) * c.size], &ONE, c.ave_con, &ONE)
                    if sim > max_sim:
                        max_sim = sim
                        sense_index = l
                        
                if c.np_value != mONEF and max_sim < c.np_value and max_sim != 0.0:
                    new_sensenum = c.index_to_sensenum[c.indexes[i]] + 1
                    if new_sensenum <= c.index_to_maxsensenum[c.indexes[i]]:
                        sense_index = new_sensenum
                        c.index_to_sensenum[c.indexes[i]] = new_sensenum
                        
                target_sense_index = c.senseindexes[i]+sense_index                
                our_saxpy(&c.size, &window_size, c.ave_con, &ONE, &c.clustervectors[target_sense_index * c.size], &ONE)
                c.sensecounts[target_sense_index] += 1
          
                for l in range(j, k):
                    if l == i:
                        continue
                    if c.indexes[l] == c.indexes[i]:
                        continue
                    if c.indexes[l] == padding_idx:
                        continue
                    if c.hs:
                        w2v_fast_sentence_sg_hs(c.points[i], c.codes[i], c.codelens[i], c.syn0, c.syn1, c.size, c.indexes[l], c.alpha, c.work, c.words_lockf, c.words_lockf_len, c.compute_loss, &c.running_training_loss)
                    if c.negative:
                        c.next_random = w2v_fast_sentence_sgcon_neg(c.negative, c.cum_table, c.cum_table_len, c.sensevectors, c.syn0,
                                                                    c.size, target_sense_index, c.indexes[l], c.alpha, c.work, c.neu1, c.neu2, c.next_random, 
                                                                    c.words_lockf, c.words_lockf_len, c.compute_loss, &c.running_training_loss)
