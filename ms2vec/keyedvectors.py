...
class KeyedVectors(utils.SaveLoad):
    ...
    def randomly_initialize_vectors(self, indexes=None, seed=0):
        ...
        for i in indexes:
            if self.global_initialize == "random":
                self.vectors[i] = pseudorandom_weak_vector(
                    self.vectors.shape[1],
                    seed_string=str(self.index_to_key[i]) + "##global" + str(seed),
                )
            elif self.global_initialize == "zeros":
                self.vectors[i] = np.zeros(self.vectors.shape[1])
        self.norms = None
                    
    def randomly_initialize_sensevectors(self, seed=0):
        """Initialize vectors with low-magnitude random vectors, as is typical for pre-trained
        Word2Vec and related models.

        """
        for i in range(len(self.index_to_sensekey)):
            self.sensevectors[i] = pseudorandom_weak_vector(
                self.sensevectors.shape[1],
                seed_string=str(self.index_to_sensekey[i]) + str(seed),
            )
        self.norms = None
    ...
    def save_sense2vec_format(
            self, fname, min_cut_count, fvocab=None, binary=False, total_vec=None, write_header=True,
            prefix='', append=False, sort_attr='count',
        ):
        """Store the input-hidden weight matrix in the same format used by the original
        C word2vec-tool, for compatibility.
...
#         keys_to_write = itertools.chain(range(0, index_id_count), store_order_vocab_keys)
        ...
        if total_vec is None:
            total_vec = 0
            for word in store_order_vocab_keys:
                for i in range(self.index_to_sensenum[self.key_to_index[word]]):
                    if self.sensecounts[self.key_to_senseindex[word] + i] >= min_cut_count:
                        total_vec += 1
        ...
        with utils.open(fname, mode) as fout:
            ...
            for word in store_order_vocab_keys:
                for i in range(self.index_to_sensenum[self.key_to_index[word]]):
                    if self.sensecounts[self.key_to_senseindex[word] + i] >= min_cut_count:
                        key = self.index_to_sensekey[self.key_to_senseindex[word] + i]
                        row = self.sensevectors[self.key_to_senseindex[word] + i]
                        if binary:
                            row = row.astype(REAL)
                            fout.write(utils.to_utf8(prefix + str(key)) + b" " + row.tobytes())
                        else:
