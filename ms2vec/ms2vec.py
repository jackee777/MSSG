...
class MSSG(utils.SaveLoad):
    def __init__(
        ...
        self.logger_a = logging.getLogger(__name__)
        fh = logging.FileHandler(filename=analysis_logfile)
        fh.setLevel(logging.INFO)
        fh.setFormatter(logging.Formatter('%(asctime)s : %(threadName)s : %(levelname)s : %(message)s'))
        self.logger_a.addHandler(fh)
        self.labels = None
    ...
    def prepare_vocab(
        ...
        if not update:
            ...
            retain_total, simple_retain_words, retain_words = 0, [], []
            # Discard words less-frequent than min_count
            if not dry_run:
                self.wv.index_to_key = []
                self.wv.index_to_sensekey = []
                self.wv.index_to_sensenum = []
                self.wv.index_to_maxsensenum = []
                # make stored settings match these applied settings
                self.min_count = min_count
                self.sample = sample
                self.wv.key_to_index = {}
                self.wv.key_to_senseindex = {}
                self.wv.key_to_maxsensenum = {}

            for word, v in self.raw_vocab.items():
                sense_num = min(self.wv.sense_num, v // self.min_count)
                if keep_vocab_item(word, v, self.effective_min_count, trim_rule=trim_rule):
                    ...
                    if v >= self.wv.min_sensecount:
                        self.wv.key_to_maxsensenum[word] = sense_num
                    else:
                        self.wv.key_to_maxsensenum[word] = 1
                    self.wv.index_to_maxsensenum.append(self.wv.key_to_maxsensenum[word])
                        
                    if self.np_value == -1:
                        self.wv.index_to_sensenum.append(self.wv.key_to_maxsensenum[word])
                    else:
                        self.wv.index_to_sensenum.append(1)
                        
                    if not dry_run:
                        simple_retain_words.append(word)
                        retain_words.append(word)
                        retain_total += v

                    for i in range(sense_num):
                        if i != 0 and v < self.wv.min_sensecount: break
                        wordname = word if i == 0 else word + self.delimiter + str(i)
                        self.wv.key_to_senseindex[wordname] = len(self.wv.index_to_sensekey)
                        self.wv.index_to_sensekey.append(wordname)                            
                else:
                    drop_unique += 1
                    drop_total += v
                    
            self.wv.index_to_sensenum = np.asarray(self.wv.index_to_sensenum, dtype=np.int8)
            self.wv.index_to_maxsensenum = np.asarray(self.wv.index_to_maxsensenum, dtype=np.int8)

            if not dry_run:
                # now update counts
                for w in simple_retain_words:
                    self.wv.set_vecattr(w, 'count', self.raw_vocab[w])
        ...
        logger.info("threshold_count=%f", threshold_count)
    ...
    def reset_weights(self):
        ...
        if self.wv.cluster_initialize == "same":
...
#         self.wv.vectors = np.asarray([self.wv.vectors[i]/np.sqrt(np.sum(self.wv.vectors[i]**2)) for i in range(len(self.wv))])
#         self.wv.bias = np.zeros(len(self.wv), dtype=REAL)
#         self.vectors_mean = np.average(self.wv.vectors, axis=0)
        
#         if self.hs:
#             self.syn1 = np.zeros((len(self.wv), self.layer1_size), dtype=REAL)
        if self.negative:
            self.syn1neg = None # unnecessary
#             self.syn1neg = np.zeros((len(self.wv), self.layer1_size), dtype=REAL)
#             self.syn1neg = premodel.syn1neg
    ...
    def _do_train_epoch(
        ...
        work, neu1, _, _, _, _, _ = thread_private_mem
        if self.sg:
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        """
        work, neu1, neu2, ave_con = inits
        tally = 0
        if self.sg:
            tally += train_batch_sg(self, sentences, alpha, work, np.copy(self.syn1neg), neu1, neu2, ave_con, self.compute_loss)
        else:
    ...
    def train(
        ...
        for cur_epoch in range(self.epochs):
            ...
            print("corpus_iterable ", corpus_iterable)
    ...
    def _job_producer(self, data_iterator, job_queue, cur_epoch=0, total_examples=None, total_words=None):
        ...
        print_log("end job producer: total sentences {}".format(data_iterator.current_sentence))
    ...
    def _get_thread_working_mem(self):
        ...
        neu2 = matutils.zeros_aligned(self.layer1_size, dtype=REAL)
        ave_con = matutils.zeros_aligned(self.layer1_size, dtype=REAL)
        return work, neu1, neu2, ave_con
    ...
    def print_loga(self, text):
#         print(text)
        self.logger_a.info(text)
